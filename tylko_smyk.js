...
let rawdata = fs.readFileSync("./tylko_smyk/sku_tylko_smyk.json");
...
async function get(url, sku, index) {
  ...
  // 	},
  // 	cookies: {
  // 		"__RequestVerificationToken": "6RY1y8FB3_MesYIQQFDQrie8H_hrYEa1RaBhtVO1xegQeoZOWtU7AGPNN7QSRmu5aILBY0R1sQ4j9fVY-tzN6lhOVFSEqP1Om7uBa_t3Nus1"
  // 	},
  // });
    ...
    .then((res) => {
      ...
      $(".complex-product").each((i, el) => {
        smyk = $(el)
          ...
          .text()
          .replace(/ zł/gi, "");
        smykTitle = $(el).find(".complex-product__name").text();
      });
    })
    .catch((err) => console.error(err));

  await new Promise((resolve) =>
    setTimeout(resolve, Math.random() * (250 - 50) + 250)
  );
  ...
  ob.price.push(links[index].kat);
  ob.price.push(links[index].grupa);
...
function writeToFile(data) {
  ...
  fs.writeFile(
    `./tylko_smyk/swieta_lista_cena_dostepnosc_${today}.txt`,
    "\ufeff" + data,
    (err) => {
      if (err) {
        console.error(err);
